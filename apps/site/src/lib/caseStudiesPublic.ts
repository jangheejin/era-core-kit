...
export function getCaseStudiesBySectorRouteSlug(routeSlug: string): CaseStudyType[] {
  const sector = sectorFromRouteSlug(routeSlug);
  if (!sector) return [];
  return getAllCaseStudies().filter((cs) => cs.sectors?.includes(sector));
}

export function getCaseStudiesByTagRouteSlug(routeSlug: string): CaseStudyType[] {
  return getAllCaseStudies().filter((cs) =>
    (cs.tags ?? []).some((t) => tagSlug(t) === routeSlug),
  );
}

export function listTagRoutes(): Array<{ slug: string; label: string; count: number }> {
